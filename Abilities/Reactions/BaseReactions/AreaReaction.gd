...
var ReactionScript = preload("res://Abilities/Reactions/BaseReactions/Reaction.gd").new()
...
func get_parent_bounding_radius() -> int:
	...
	for point in ReactionScript.parents["source"].get_node("CollisionPolygon2D").polygon:
